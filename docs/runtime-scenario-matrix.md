...
| 4. ContextPack present | ContextPack active, no fabricated metrics/anchors when evidence is missing | `Runtime scenario matrix (deterministic) -> Scenario 4: ContextPack signals avoid fabricated metrics and missing anchors` + ContextPack panel tests in `src/app/App.ui.test.tsx` |
