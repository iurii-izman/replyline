...
pub async fn transcribe_wav(
    ...
    let response = {
        ...
        for attempt in 0..=MAX_RETRIES {
            if attempt > 0 {
                tokio::time::sleep(Duration::from_millis(RETRY_BASE_MS * 2u64.pow(attempt - 1)))
                    .await;
            }
            match client
                .post(&endpoint)
...
pub async fn transcribe_pcm_streaming(
    ...
    .await
    .map_err(|_| "STT_WS_TIMEOUT: Deepgram WS connect timed out.".to_string())?
    .map_err(|err| format!("STT_WS_CONNECT_FAILED: Deepgram WS connect failed: {err}"))?;
    ...
    for chunk in pcm_bytes.chunks(3_200) {
        socket
            ...
            .await
            .map_err(|err| format!("STT_WS_SEND_FAILED: Deepgram WS send failed: {err}"))?;
    }
    ...
    socket
        ...
        .await
        .map_err(|err| format!("STT_WS_CLOSE_FAILED: Deepgram WS close message failed: {err}"))?;
    ...
    loop {
        let next = timeout(Duration::from_secs(WS_READ_TIMEOUT_SECS), socket.next())
            .await
            .map_err(|_| "STT_WS_TIMEOUT: Deepgram WS response timed out.".to_string())?;

        let Some(frame) = next else {
            break;
        };

        match frame.map_err(|err| format!("STT_WS_READ_FAILED: Deepgram WS read failed: {err}"))? {
            Message::Text(text) => {
    ...
    if transcript.is_empty() {
        return Err("STT_EMPTY: Deepgram streaming returned an empty transcript.".to_string());
    }
