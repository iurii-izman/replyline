...
fn repair_answer_now_inner(value: &str, transcript: &str) -> String {
    ...
    if clean.ends_with('?') {
        return format!(
            "{clean} После уточнения сразу зафиксирую решение, владельца и ближайший срок в рабочем канале. Это позволит всем участникам синхронизироваться и избежать повторных согласований."
        );
    }
    ...
    if contains_any(&lower, &["когда", "срок", "deadline", "дедлайн", "дата"]) {
        match variant {
            0 => "Предлагаю зафиксировать срок прямо сейчас: назовите финальную дату и владельца подтверждения. Если дата сдвигается, сразу обозначим новый дедлайн и формат апдейта до конца дня. Все заинтересованные получат уведомление об изменении сроков и смогут оперативно скорректировать свои планы.".to_string(),
            1 => "Давайте сверим дедлайн в этом треде: какая финальная дата и кто дает письменное подтверждение сегодня. После согласования отправлю краткий итог с владельцем и контрольной точкой всем участникам цепочки. Если нужна дополнительная информация, оперативно запрошу у ответственных и добавлю в документ.".to_string(),
            _ => "Нужна конкретика по сроку: какую дату берем и кто фиксирует решение. Я подготовлю короткий итог с датой, владельцем и следующим чекпоинтом. Команда получит понятную картину и сможет скорректировать свои задачи при необходимости без дополнительных согласований.".to_string(),
        }
    } else if contains_any(&lower, &["кто", "владел", "owner", "ответствен"]) {
        match variant {
            0 => "Давайте назначим владельца шага и время статус-апдейта. Я зафиксирую ответственного, срок и формат следующего контроля в одном сообщении. Если владелец не подтвердится в течение дня, предложу альтернативного кандидата и сразу согласую с командой.".to_string(),
            1 => "Предлагаю прямо сейчас определить ответственного и дедлайн. После этого отправлю краткий итог: кто ведет, что сдаем и когда сверяемся. Это стандартная практика, которая помогает держать все задачи под контролем и избегать потери договоренностей.".to_string(),
            _ => "Нужно прояснить владельца и срок. Я оформлю это письменно: ответственный, следующий шаг и контрольная дата. Если ответственный не может взять задачу, обсудим альтернативное распределение в рабочем канале и зафиксируем новое решение.".to_string(),
        }
    } else {
        match variant {
            0 => "Предлагаю зафиксировать один конкретный шаг, владельца и срок до конца дня. После согласования отправлю короткий итог в рабочий канал. Если нужны дополнительные вводные, соберу их у команды и добавлю в итоговый документ для всех заинтересованных.".to_string(),
            1 => "Давайте уточним следующий артефакт и время проверки сегодня. Я подготовлю краткий follow-up с решением и ответственным. После этого можно будет двигаться дальше с чётким пониманием следующего шага и распределением ответственности.".to_string(),
            _ => "Сверим владельца, дедлайн и формат апдейта в этом обсуждении. Затем зафиксирую итог письменно, чтобы не потерять договоренность. Если потребуется согласование с другими командами, сразу обозначу и это в документе для общей прозрачности.".to_string(),
        }
    }
}
...
fn limits_for_transcript(transcript: &str, profile: &AnswerProfileConfig) -> CardLimits {
    ...
    let answer_chars = match chars_band(transcript) {
        "short" => 800usize,
        "medium" => 1400usize,
        _ => 1800usize,
    };
...
mod tests {
    ...
    fn maps_v3_to_legacy_fields() {
        let raw = v3_json(
            ...
            "Давайте зафиксируем владельца и срок: я пришлю обновление сегодня до 17:00. После согласования отправлю краткий итог в чат. В случае задержки сразу обозначим новый дедлайн и формат апдейта. Коллеги получат понятную картину и смогут скорректировать свои задачи.",
    ...
    fn repairs_vague_next_step() {
        let raw = legacy_json(
            ...
            "Давайте согласуем приоритеты и срок сегодня до 17:00. Я подготовлю короткий итог и отправлю его в чат до конца дня. Это поможет всем участникам синхронизироваться и принять взвешенное решение без лишних задержек.",
            "Потом посмотрим.",
        );
        let (card, quality) =
    ...
    fn factual_follow_up_is_preserved_without_work_fallback() {
        let raw = v3_json(
            ...
            "Столицей Франции является Париж. Это один из самых известных городов мира. Город расположен на реке Сене и ежегодно привлекает миллионы туристов со всего света.",
    ...
    fn partial_v3_extract() {
        let broken = r#"Here: {"question_brief": "Risk", "answer_now": "Test value paragraph one. Test value paragraph two action today. Extra sentence to reach minimum word count for the new richer answer profiles.", "star_evidence": "heard risk", "next_step": "Send email with owner"}"#;
    ...
    fn legacy_regression_accepts_concrete_card() {
        let raw = legacy_json(
            ...
            "Давайте сейчас зафиксируем владельца и срок: я пришлю обновление сегодня до 17:00. После этого отправлю краткий итог в чат. Если срок изменится, я сразу сообщу всем заинтересованным участникам и предложу альтернативный план действий.",
            "Отправлю письмо с владельцем и чекпоинтом на завтра.",
        );
        let (card, _) =
            normalize_parsed_card(&raw, "", crate::prompt_registry::default_answer_profile())
    ...
    fn accepts_direct_answer_without_mandatory_clarifier() {
        let raw = legacy_json(
            ...
            "Да, укладываемся: сегодня до 17:00 закрываю проверку и подтверждаю финальную дату. После этого отправлю короткий итог в чат с владельцем и контрольной точкой. Команда уже в курсе статуса и готова к финальному подтверждению.",
    ...
    fn preserves_direct_factual_answer_without_work_action_repair() {
        let raw = v3_json(
            ...
            "Городом влюбленных обычно называют Париж. Это устойчивое культурное обозначение города благодаря его романтическому образу, сложившемуся в литературе и кинематографе на протяжении многих десятилетий. Туристы со всего мира приезжают сюда именно за этой неповторимой атмосферой.",
    ...
    fn keeps_evidence_and_risk_out_of_spoken_answer() {
        let raw = r#"{"question_brief":"Нужно уточнить термин в вопросе.","answer_now":"Не хочу угадывать значение без контекста. Уточните, пожалуйста, что именно имеется в виду, и я отвечу по существу. Это позволит избежать недопонимания и даст наиболее точный и релевантный ответ.","star_evidence":"Прыгать.","next_step":"Уточнить вопрос в чате перед ответом.","risk_or_clarifier":"Что именно означает «прыгать» в этом контексте?"}"#;
        let (card, _) = normalize_parsed_card(
            raw,
            "Прыгать.",
            crate::prompt_registry::default_answer_profile(),
    ...
    fn allows_clarifier_when_ambiguity_blocks_direct_response() {
        let raw = legacy_json(
            "Запрос расплывчатый.",
            "Чтобы не промахнуться, уточню: вы ждете закрытия по сроку, по качеству или по владельцу? После ответа сразу зафиксирую решение и дедлайн в чате. Если критерии ещё не определены, помогу их сформулировать в этом же обсуждении.",
