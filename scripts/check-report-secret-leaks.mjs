...
const scanRoots = ["reports", "docs", ".env.docker.example"];
...
function listFiles(inputPath) {
  ...
  for (const entry of entries) {
    out.push(...listFiles(resolve(abs, entry)));
  }
  return out;
}

const files = scanRoots.flatMap((root) => listFiles(root));
const violations = [];
...
console.log(
  `[report-secret-leaks] OK: scanned ${files.length} files in reports/docs/.env.docker.example`,
);
