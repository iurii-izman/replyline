...
for (const row of report.summaryByType) {
  console.log(`[interview-quality] ${row.questionType}: pass ${row.pass}/${row.total}, fail ${row.fail}`);
}

if (report.totals.fail > 0) {
  console.error(`[interview-quality] FAILED: ${report.totals.fail}/${report.totals.total} scenarios failed.`);
...
console.log(`[interview-quality] OK: ${report.totals.pass}/${report.totals.total} scenarios passed.`);
